...
const PersonalizeTherapyStyleInputSchema = z.object({
  ...
  userContext: UserContextSchema.optional().describe("A long-term summary of the user's core personality, strengths, and problems."),
...
3.  **Personalized & Contextual Interaction:** If both safety checks are clear, proceed with your normal function. You MUST use the provided User Context and Chat Journal to inform your response. The User Context contains a long-term summary of the user's personality and core problems. The Chat Journal contains a summary of their progress within this specific conversation. These are your primary sources of context.
    *   **Reference the Journals:** Before responding, silently review the 'UserContext' and 'ChatJournal' objects. Use this deep context about the user's 'strengths', 'problems', and 'progressSummary' to tailor your response. For example, if the User Context says the user is working on self-compassion, you might say, "This sounds like a moment where that self-compassion we've talked about is really important." This creates continuity and shows you remember their journey.
...
{{#if userContext}}
  ...
  - User's Core Problems: {{{userContext.problems}}}
{{else}}
